...
base_url ='https://tunovelaligera.com/swallowed-star/ss-volumen-5-capitulo-60-hong-y-la-vid-larga/'
...
#Escribe archivo
def arch():
    global arch_text, nombre_serie, archivo_actual, numero_capitulo, volumen
    numero_cap_txt = numero_capitulo
    arch_txt = open(f'-{nombre_serie}_{volumen}.{numero_capitulo}.txt', 'a', encoding='utf-8')
    return arch_txt
arch_txt= arch()
    ...
    try:
        historia_element = soup.find('div', class_=f"entry-content_wrap_s_{entrada[0][0]}")
        	...
        	try:
        		...
        		try:
        			historia_element = soup.find('div', class_=f"entry-content_wrap_s_{entrada[0]}")
        		except:
        			historia_element = soup.find('div', class_=f"entry-content_wrap_s_{entrada}")
...
"""
    ...
    enlace_dos= soup.find('a', href=True, text="Siguiente Capítulo")['href']
    ...
    if section_match:
        ...
        print(f"Enlace al siguiente capítulo: {enlace_dos}")
        base_url = enlace_dos
        print(base_url)
        time.sleep(5)
    else:
